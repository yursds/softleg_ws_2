import numpy as np
import rclpy
from rclpy                      import logging, clock
...
# TO DO: mijerk trajectory for getup

class GetUp(Node):
    
    def __init__(self):
        
        super().__init__(node_name='getup_controller')
        
        # Init vars
        self.simulation  = False
        self.deltaT      = 5.0
        self.rate        = 100
        self.default_dof = np.array([-3.4, 2.4])
        ...
        logging.get_logger('rclpy.node').info(f'joint pos: {self.init_pos}')
        ...
        logging.get_logger('rclpy.node').info('Getting Home.') 
    ...
    def getup_callback(self):
        ...
        logging.get_logger('rclpy.node').info(f'joint pos: {self.joint_pos}') 
        logging.get_logger('rclpy.node').info(f'i: {self.i}') 
        ...
        joint_msg.header.stamp = clock.Clock().now().to_msg()
    ...
    def joint_state_callback(self, msg):
        ...
        for i in range(0, self.njoint):
            ...
            logging.get_logger('rclpy.node').info('{}'.format(msg.position[:].tolist()))
            # logging.get_logger('rclpy.node').info('{}'.format(self.init_pos))
        self.default_acquired = True
        logging.get_logger('rclpy.node').info('Joints acquired, starting getup') 
