...
class Command_Generator(Node):
    ...
    def __init__(self):
        ...
        super().__init__(node_name='command_trajectory_node')
        ...
        self.uRL_old        = torch.zeros(self.njoint,)
        self.uILC_old       = torch.zeros(self.njoint,)
        ...
        self.ilc_ctrl = ILC_base(dimU=self.njoint, samples=samples, Le = torch.tensor(0), Lde = torch.tensor(0), Ldde = torch.tensor(0))
        ...
        # logging
        self.get_logger().info(f'Loading model from {self.model_rl_path}')
    ...
    def _parser_parameter_node(self):
        ...
        self.declare_parameter('topic.feedback', '/command')
        self.declare_parameter('task.duration', 2.0)
        self.declare_parameter('task.wait_t', 5.0)
        self.declare_parameter('task.pf', [0.0, 0.0])
        self.declare_parameter('model_rl_path', abs_path+'/../models/rl_classic/best_model.zip')
    ...
    def _build_pub_sub(self):
        ...
        self.ref_sub      = self.create_subscription(JointState, self.topic_joint_state, self.joint_state_callback, qos)
    ...
    def command_callback(self):
        ...
        command = torch.zeros(self.njoint).flatten().tolist()
        ...
        if self.get_first_state:
            ...
            # start trajectory and use RL and ILC
            if delta_t < self.duration and delta_t >= 0.0:
                ...
                r_, dr_, _ = self._minsnap(qi = self.qi, qf = self.qf, duration = self.duration, t = delta_t)
            ...
            if msg.name[i] in self.joint_names:
                ...
                all(isinstance(value, float) for value in msg.velocity):
                    ...
                    if not brutal_fl:
                        ...
                        self.joint_acc[msg.name[i]] = 0.0
                        self.get_first_state        = True
                    else:
                        # brutal derivative computation
    ...
    def rlilc_callback(self):
        ...
        self.uRL_msg.effort = self.uRL.flatten().tolist()
