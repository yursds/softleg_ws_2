...
class Command_Generator(Node):
    ...
    def __init__(self):
        ...
        ldde = torch.tensor(self.ldde*self.rate_policy)
        lde  = torch.tensor(self.lde*self.rate_policy)
        le   = torch.tensor(self.le*self.rate_policy)
        ...
        self.joint_kp = np.array([0.025*2, 0.025])/10
        self.joint_kd = np.array([0.25, 0.25])*4/1
    ...
    def _parser_parameter_node(self):
        ...
        self.declare_parameter('controller_gain.ilc.le', 0.0002)
        self.declare_parameter('controller_gain.ilc.lde', 0.0004)
        self.declare_parameter('controller_gain.ilc.ldde', 0.0008)
        ...
        self.le                = self.get_parameter('controller_gain.ilc.le').get_parameter_value().double_value
        self.lde               = self.get_parameter('controller_gain.ilc.lde').get_parameter_value().double_value
        self.ldde              = self.get_parameter('controller_gain.ilc.ldde').get_parameter_value().double_value
    ...
    def _step_rlilc(self, delta_t:float):
        ...
        # ================00
        # NOTE
        uFB = self.uTot_old - self.uMB - self.uRL - self.uILC
        # self.get_logger().info(f'e: {(e_)}')
        # self.get_logger().info(f'kp: {(self.joint_kp)}')
        # self.get_logger().info(f'kp*e: {torch.mul(torch.from_numpy(self.joint_kp), e_.flatten())}')
        # self.get_logger().info(f'uFB_fake: {uFB} and uFB: {uFB_real.flatten()}')
        # self.get_logger().info(f'uFB_fake: {uFB.flatten()} and uFB: {uFB_real.flatten()}')
        
        # uFB_fake = torch.mul(15*(torch.from_numpy(self.joint_kp)).view(-1,1), e_).type(torch.float32) \
        #     + torch.mul((torch.from_numpy(self.joint_kd)).view(-1,1),de_).type(torch.float32)
        
        u_delta_ = uFB  + self.uILC + self.uRL       # consider u_delta = PD + ILC + RL 
        ...
        if delta_t_fut < self.duration:
            ...
            obs_list  = torch.cat([
                ...
                self.uRL.flatten(), url_old_ep.flatten(), uilc_old_ep.flatten(), ufb_old_ep.flatten()], dim=0)
        ...
        if self.get_first_state:
            ...
            if self.leader:
                ...
                # start trajectory and use RL and ILC if the node is leader
                    ...
                    command_tot   = (G_vec + uRL_interp + uILC_interp).flatten().tolist() #_interp + self.uILC_interp
